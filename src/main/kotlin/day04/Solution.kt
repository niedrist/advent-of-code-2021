...
fun main() {
    ...
    val bingos = bingoNumbers.withIndex().groupBy { it.index / BINGO_SIZE }.map { it.value.map { it.value } }.map { Bingo(it.toMutableList(), false) }
    println(part1(draws, bingos))
    println(part2(draws, bingos))
}

fun part1(draws: List<Int>, bingos: List<Bingo>): Int {
    for (draw in draws)
        for (bingo in bingos) {
...
fun part2(draws: List<Int>, bingos: List<Bingo>): Int {
